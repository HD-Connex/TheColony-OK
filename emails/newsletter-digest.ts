export interface DigestProps {
  ...
  intro?: string; // Phase 2 AI: personalized Claude summary for subscriber's counties/topics (or fallback generic)
}

export function newsletterDigestHtml({ subject, items, unsubscribeUrl, intro }: DigestProps): string {
  ...
  const introHtml = intro ? `<p style="margin:0 0 12px;font-size:13px;border-left:3px solid #b22222;padding-left:8px;">${intro}</p>` : "";
  return `<!doctype html>
